...
class SIG17_Training_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ldr0 = ldr_images[0].astype(np.float32).transpose(2, 0, 1)  
        ldr1 = ldr_images[1].astype(np.float32).transpose(2, 0, 1)
        ldr2 = ldr_images[2].astype(np.float32).transpose(2, 0, 1)
        ldr0 = torch.from_numpy(ldr0) 
        ldr1 = torch.from_numpy(ldr1) 
        ldr2 = torch.from_numpy(ldr2) 
...
class SIG17_Validation_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.crop:
            ...
            ldr0 = ldr_images[0][x:x + self.crop_size, y:y + self.crop_size].astype(np.float32).transpose(2, 0, 1)  # -->[C,H,W]
            ldr1 = ldr_images[1][x:x + self.crop_size, y:y + self.crop_size].astype(np.float32).transpose(2, 0, 1)
            ldr2 = ldr_images[2][x:x + self.crop_size, y:y + self.crop_size].astype(np.float32).transpose(2, 0, 1)
        else:
            img0 = pre_img0.astype(np.float32).transpose(2, 0, 1)
            img1 = pre_img1.astype(np.float32).transpose(2, 0, 1)
            img2 = pre_img2.astype(np.float32).transpose(2, 0, 1)
...
class Img_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ldr0 = self.ldr_patches[index][0].astype(np.float32).transpose(2, 0, 1)  
        ldr1 = self.ldr_patches[index][1].astype(np.float32).transpose(2, 0, 1)
        ldr2 = self.ldr_patches[index][2].astype(np.float32).transpose(2, 0, 1)
        ldr0 = torch.from_numpy(ldr0) 
        ldr1 = torch.from_numpy(ldr1)  
        ldr2 = torch.from_numpy(ldr2) 
